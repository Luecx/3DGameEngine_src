...
public abstract class ComplexGameObject {
     ...
     */
    public synchronized void setScale(Vector3f scalation) {
     ...
     */
    public synchronized void increasePosition(Vector3f a) {
        this.position.x += a.x;
        this.position.y += a.y;
        this.position.z += a.z;
        setOutdated();
     ...
     */
    public synchronized void increaseScale(Vector3f a) {
        this.scale.x += a.x;
        this.scale.y += a.y;
        this.scale.z += a.z;
        setOutdated();
     ...
     */
    public synchronized Matrix4f getClonedTransformationMatrix() {
        if (outdated) {
            outdated = false;
            this.transformationMatrix = this.createTransformationMatrix();
        }
     ...
     * @return      -the transformation matrix
     */
    private synchronized Matrix4f createTransformationMatrix() {
